...
@section('content')
                ...
                <div class="card-body">
                    ...
                    <form method="POST" id="invoice-form" action="{{ route('invoices.purchase.update', urlencode($invoice->invoice_number)) }}">
                        ...
                        <div class="row">
                            ...
                            <input type="hidden" name="penerimaan_number" value="{{ $invoice->penerimaan_number }}">
                        </div>

                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <label>No Terima</label>
                                <input type="text" class="form-control" value="{{ $invoice->penerimaan_number }}"
                                    readonly>
                            </div>
                            <div class="col-md-6 mb-3">
                                <label>Supplier</label>
                                <input type="text" class="form-control"
                                    value="{{ $invoice->penerimaan->supplier->supplier_name ?? '-' }}" readonly>
                            </div>
                        </div>

                        <div class="mb-3">
                            <label>PPN Status</label>
                            <input type="text" class="form-control" value="{{ $invoice->penerimaan->ppn_status }}"
                                readonly>
                        </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered align-middle text-center" id="invoice-table">
                                ...
                                <tbody>
                                    @foreach ($invoice->penerimaan->details as $i => $detail)
                                        @php
                                            $product = $detail->product;
                                            $matched = $invoice->details
                                                ->where('penerimaan_detail', $detail->id)
                                                ->first();
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const ppnStatus = '{{ $invoice->penerimaan->ppn_status }}';
